      ...
      isRunning(true) 
{
    ...
    try {
        // Co gang tai tat ca du lieu (bao gom ca accounts)
        FileIO::loadAllData(ql, accountManager, "./data"); 
    } catch (...) {
        // Neu ham loadAllData cua ban bi loi (vi thieu AccountManager)
        // No se duoc bat o day, va code van tiep tuc
        std::cerr << "Loi: Khong the loadAllData. Co the do FileIO cu." << std::endl;
        // Ban hay cap nhat FileIO.h/.cpp ma toi da gui
    }
    
    // (QUAN TRONG) Kiem tra xem admin co ton tai khong (sau khi load)
    if (accountManager.getAccount("admin") == nullptr) {
        // Neu khong co admin (chay lan dau HOAC load loi), tao moi
        std::cout << "Khong tim thay 'admin', dang tao moi voi mat khau '123'..." << std::endl;
        accountManager.addAccount("admin", "123", AccountType::ADMIN, nullptr);
