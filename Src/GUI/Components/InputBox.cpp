...
void InputBox::setPosition(float x, float y)
{
    ...
    if (text)
        text->setPosition(sf::Vector2f(box.getPosition().x + 10, textY));
    if (placeholder)
        placeholder->setPosition(sf::Vector2f(box.getPosition().x + 10, textY));
    updateCursorVisual();
}
...
{
    if (!isFocused)
        return; // Chi xu ly neu dang duoc focus
